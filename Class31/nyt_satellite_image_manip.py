...
#     pixels.sort() ### Sorts by red first, not what we want
    ...
    for pixel in pixels:
        ...
        if lum in luminance_freq:
            luminance_freq[lum] += 1
        else:
            luminance_freq[lum] = 1
        
    pixels_with_luminance.sort()
    
    print(luminance_freq)
    
    luminances = [] # luminances between 0 and 255
    luminance_freq_list = []
    ...
    for lum in range(256):
        ...
        if lum in luminance_freq:
            freq = luminance_freq[lum]
        else:
            freq = 0
        
        luminance_freq_list.append(freq)
